...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!ratingDoc.exists) {
      return new Response(JSON.stringify({ error: "Rating introuvable" }), {
        status: 404,
    ...
    return new Response(JSON.stringify({ error: "Erreur lors de la récupération du rating" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    })
  }
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return new Response(JSON.stringify({ error: "Erreur lors de la suppression du rating" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    })
  }
