  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button type="button" onClick={() => setIsCreateOpen(true)} className="rounded-full bg-accent px-4 py-2 text-sm font-semibold text-bg transition hover:opacity-90">
          + Ajouter un produit
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-bg/80 px-4 py-3">
        ...
        <input value={searchTerm} onChange={(e) => { setPage(1); setSearchTerm(e.target.value) }} placeholder="Nom, description" className="w-60 rounded-lg border border-border bg-card px-3 py-2 text-sm" />

        <label className="text-sm font-semibold">Mood</label>
        <input value={moodFilter === "all" ? "" : moodFilter} onChange={(e) => { setPage(1); setMoodFilter(e.target.value.trim() ? e.target.value : "all") }} placeholder="HAPPY, SAD..." className="w-40 rounded-lg border border-border bg-card px-3 py-2 text-sm" />

        <label className="text-sm font-semibold">Prix min</label>
        <input type="number" min="0" step="0.01" value={minPrice} onChange={(e) => { setPage(1); setMinPrice(e.target.value) }} className="w-28 rounded-lg border border-border bg-card px-3 py-2 text-sm" />
        <label className="text-sm font-semibold">Prix max</label>
        <input type="number" min="0" step="0.01" value={maxPrice} onChange={(e) => { setPage(1); setMaxPrice(e.target.value) }} className="w-28 rounded-lg border border-border bg-card px-3 py-2 text-sm" />

        <label className="text-sm font-semibold">Tri 1</label>
        <select value={primaryField} onChange={(e) => { setPage(1); setPrimaryField(e.target.value as SortField) }} className="rounded-lg border border-border bg-card px-3 py-2 text-sm">
          <option value="name">Nom</option>
          <option value="mood">Mood</option>
          <option value="price">Prix</option>
        </select>
        <select value={primaryDir} onChange={(e) => { setPage(1); setPrimaryDir(e.target.value as SortDir) }} className="rounded-lg border border-border bg-card px-3 py-2 text-sm">
          <option value="asc">Asc</option>
          <option value="desc">Desc</option>
        </select>

        <label className="text-sm font-semibold">Tri 2</label>
        <select value={secondaryField} onChange={(e) => { setPage(1); setSecondaryField(e.target.value as SortField | "none") }} className="rounded-lg border border-border bg-card px-3 py-2 text-sm">
          <option value="none">Aucun</option>
          <option value="name">Nom</option>
          <option value="mood">Mood</option>
          <option value="price">Prix</option>
        </select>
        <select value={secondaryDir} onChange={(e) => { setPage(1); setSecondaryDir(e.target.value as SortDir) }} disabled={secondaryField === "none"} className="rounded-lg border border-border bg-card px-3 py-2 text-sm">
          <option value="asc">Asc</option>
          <option value="desc">Desc</option>
        </select>

        <label className="text-sm font-semibold">Par page</label>
        <select value={pageSize} onChange={(e) => { setPage(1); setPageSize(Number(e.target.value)) }} className="rounded-lg border border-border bg-card px-3 py-2 text-sm">
        ...
        <table className="min-w-full divide-y divide-border text-sm">
          ...
          <tbody className="divide-y divide-border bg-card">
            {!loading && products.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-muted">Aucun produit trouvé</td>
              </tr>
            ) : (
              products.map((product) => (
                <tr key={product.id} onClick={() => handleRowClick(product.id)} className="cursor-pointer transition hover:bg-bg/80">
                  <td className="px-4 py-3">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-10 w-10 rounded-lg object-cover" loading="lazy" /> : <div className="h-10 w-10 rounded-lg border border-border bg-bg/60" />}</td>
                  <td className="px-4 py-3">{product.name}</td>
                  <td className="px-4 py-3"><span className={getMoodBadgeClass(product.mood)}>{product.mood}</span></td>
      ...
      <div className="flex items-center justify-between rounded-2xl border border-border bg-bg/70 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setPage((prev) => Math.max(1, prev - 1))} disabled={page <= 1} className="rounded-full border border-border px-4 py-2 text-sm font-semibold disabled:opacity-50">Précédent</button>
          <button type="button" onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))} disabled={page >= totalPages} className="rounded-full border border-border px-4 py-2 text-sm font-semibold disabled:opacity-50">Suivant</button>
        </div>
      </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-[min(92vw,460px)] rounded-2xl border border-border bg-card p-6 shadow-xl">
            ...
            <p className="mt-3 text-sm text-muted">{deleteTarget.name} — {deleteTarget.price.toFixed(2)} €</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-[min(92vw,520px)] rounded-2xl border border-border bg-card p-6 shadow-xl">
            ...
            <form className="mt-6 space-y-4" onSubmit={handleCreateSubmit}>
              <input type="text" value={createForm.name} onChange={(e) => setCreateForm((p) => ({ ...p, name: e.target.value }))} placeholder="Nom" className="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm" required />
              <input type="text" value={createForm.mood} onChange={(e) => setCreateForm((p) => ({ ...p, mood: e.target.value }))} placeholder="Mood" className="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm" required />
              <input type="number" step="0.01" min="0" value={createForm.price} onChange={(e) => setCreateForm((p) => ({ ...p, price: e.target.value }))} placeholder="Prix" className="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm" required />
              <textarea value={createForm.description} onChange={(e) => setCreateForm((p) => ({ ...p, description: e.target.value }))} placeholder="Description" className="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm" rows={4} />
              <input type="file" accept="image/*" onChange={handleImageChange} className="w-full text-sm" />
              {imagePreview && <img src={imagePreview} alt="Aperçu" className="h-24 w-24 rounded-lg object-cover" />}
